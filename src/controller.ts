...
import { WorkingTheme, emptyTheme, buildThemeFile, TextMateRule } from "./themeModel";
...
/**
 * The ThemePaint editor logic, decoupled from where it is hosted. The same
 * controller drives both the sidebar webview view and the editor panel: it owns
 * the working theme, talks to the webview, and is the only thing that writes
 * settings (via SettingsWriter).
 */
export class ThemePaintController {
  ...
  constructor(
    ...
    this.disposables.push(this.webview.onDidReceiveMessage((m) => this.handleMessage(m)));
  }
    ...
    try {
      switch (msg?.type) {
        ...
        case "addAdvancedRule": {
          const rule: TextMateRule = { scope: String(msg.scope || "").trim(), settings: {} };
          if (msg.foreground) {
  ...
  private pushSavedThemes() {
    this.post({ type: "savedThemesUpdated", savedThemes: this.store.list() });
  }
  ...
  private async doExport() {
    ...
    const result = await exportTheme(
      target,
      this.theme,
      this.context.extension.packageJSON.version || "0.0.1"
    );
    this.post({ type: "exportResult", result });
  ...
  private async runInspectScopes() {
    this.status("info", await inspectScopesGuide());
  }

  private async runPickFromCursor() {
    this.post({ type: "cursorToken", info: await semanticTokenAtCursor() });
  }
  ...
  private contrastResults() {
    const colorOf = (ref: string): string | undefined => {
      if (ref.startsWith("@token:")) {
        return this.theme.tokens[ref.slice("@token:".length)]?.foreground;
      }
